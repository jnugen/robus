/***********************************************************************
 * @file		lpc17xx_uart.h
 * @brief		Contains all macro definitions and function
 *			prototypes support for UART firmware library
 *			on LPC17xx
 * @version		3.0
 * @date		18. June. 2010
...
/*!< If direction control is enabled (bit DCTRL = 1),
...
/*!< This bit reverses the polarity of the direction control signal on
 * the RTS (or DTR) pin. The direction control pin will be driven to
 * logic "1" when the transmitter has data to be sent */
...
#define PARAM_UART_STOPBIT(stopbit) \
...
#define PARAM_UART_FIFO_LEVEL(fifo) \
  (((fifo)==UART_FIFO_TRGLEV0) || ((fifo)==UART_FIFO_TRGLEV1) || \
   ((fifo)==UART_FIFO_TRGLEV2) || ((fifo)==UART_FIFO_TRGLEV3))
...
/** Macro to check the input UART1_INTCFG parameters -
 * expansion input parameter for UART1 */
#define PARAM_UART1_INTCFG(IntCfg) \
  (((IntCfg)==UART1_INTCFG_MS) || ((IntCfg)==UART1_INTCFG_CTS))

/** Macro to check the input UART_AUTOBAUD_MODE parameters */
#define PARAM_UART_AUTOBAUD_MODE(ABmode) \
  (((ABmode)==UART_AUTOBAUD_MODE0) || ((ABmode)==UART_AUTOBAUD_MODE1))
...
#define PARAM_UART_IrDA_PULSEDIV(PulseDiv) \
  (((PulseDiv)==UART_IrDA_PULSEDIV2) || \
   ((PulseDiv)==UART_IrDA_PULSEDIV4) || \
   ((PulseDiv)==UART_IrDA_PULSEDIV8) || \
   ((PulseDiv)==UART_IrDA_PULSEDIV16) || \
   ((PulseDiv)==UART_IrDA_PULSEDIV32) || \
   ((PulseDiv)==UART_IrDA_PULSEDIV64) || \
   ((PulseDiv)==UART_IrDA_PULSEDIV128) || \
   ((PulseDiv)==UART_IrDA_PULSEDIV256))

/* Macro to check the input UART1_SignalState parameters */
#define PARAM_UART1_SIGNALSTATE(x) \
  (((x)==INACTIVE) || ((x)==ACTIVE))

/** Macro to check the input PARAM_UART1_MODEM_PIN parameters */
#define PARAM_UART1_MODEM_PIN(x) \
  (((x)==UART1_MODEM_PIN_DTR) || ((x)==UART1_MODEM_PIN_RTS))

/** Macro to check the input PARAM_UART1_MODEM_MODE parameters */
#define PARAM_UART1_MODEM_MODE(x) \
  (((x)==UART1_MODEM_MODE_LOOPBACK) || \
   ((x)==UART1_MODEM_MODE_AUTO_RTS) || \
...
#define PARAM_UART_IrDA(x) \
  (((uint32_t *)(x))==((uint32_t *)LPC_UART3))
#define PARAM_UART1_MODEM(x) \
  (((uint32_t *)(x))==((uint32_t *)LPC_UART1))

/** Macro to check the input value for
 * UART1_RS485_CFG_MATCHADDRVALUE parameter */
#define PARAM_UART1_RS485_CFG_MATCHADDRVALUE(x) \
  (((x)<0xFF))

/** Macro to check the input value for
 * UART1_RS485_CFG_DELAYVALUE parameter */
#define PARAM_UART1_RS485_CFG_DELAYVALUE(x) \
  (((x)<0xFF))
 ...
 */
typedef enum {
    UART_FIFO_TRGLEV0 = 0,	/*!< UART FIFO trig level 0: 1 character */
    UART_FIFO_TRGLEV1, 		/*!< UART FIFO trig level 1: 4 character */
    UART_FIFO_TRGLEV2,		/*!< UART FIFO trig level 2: 8 character */
    UART_FIFO_TRGLEV3		/*!< UART FIFO trig level 3: 14 character */
...
typedef enum {
    ...
    UART1_INTCFG_CTS,	/*!< CTS1 signal transition int. en. (UART1 only) */
...
uint32_t UART_Send(LPC_UART_TypeDef *UARTx,
  uint8_t *txbuf, uint32_t buflen, TRANSFER_BLOCK_Type flag);
uint32_t UART_Receive(LPC_UART_TypeDef *UARTx,
  uint8_t *rxbuf, uint32_t buflen, TRANSFER_BLOCK_Type flag);
...
void UART_IntConfig(LPC_UART_TypeDef *UARTx,
  UART_INT_Type UARTIntCfg, FunctionalState NewState);
...
void UART_ABCmd(LPC_UART_TypeDef *UARTx,
  UART_AB_CFG_Type *ABConfigStruct, FunctionalState NewState);

/* UART1 FullModem functions ---------------------------------------------*/
void UART_FullModemForcePinState(LPC_UART1_TypeDef *UARTx,
  UART_MODEM_PIN_Type Pin, UART1_SignalState NewState);
void UART_FullModemConfigMode(LPC_UART1_TypeDef *UARTx,
  UART_MODEM_MODE_Type Mode, FunctionalState NewState);
