/***********************************************************************
 * @file		lpc17xx_uart.c
 * @brief		Contains all functions support for UART firmware
			library on LPC17xx
 * @version		3.0
 * @date		18. June. 2010
 ...
 * configuration file in each example directory ("lpc17xx_libcfg.h")
 * must be included, otherwise the default FW library configuration 
 * file must be included instead.
 */
...
static Status uart_set_divisors(
  ...
  uint32_t baudrate)
{
    ...
    /* In the Uart IP block, baud rate is calculated using FDR and
     * DLL-DLM registers
     * The formula is :
     * BaudRate = uClk * (mulFracDiv/(mulFracDiv+dividerAddFracDiv) /
     *                                  (16 * (DLL)
     * It involves floating point calculations. That's the reason the
     * formulae are adjusted with Multiply and divide method. */
    /* The value of mulFracDiv and dividerAddFracDiv should comply to
     * the following expressions:
     * 0 < mulFracDiv <= 15, 0 <= dividerAddFracDiv <= 15 */
    for (mulFracDiv = 1; mulFracDiv <= 15;mulFracDiv++) {
	for (dividerAddFracDiv = 0;
	  dividerAddFracDiv <= 15; dividerAddFracDiv++) {
	...
	if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1) 	{
	    ...
	    ((LPC_UART1_TypeDef *)UARTx)->/*DLIER.*/DLM =
	      UART_LOAD_DLM(diviserOptimal);
	    ((LPC_UART1_TypeDef *)UARTx)->/*RBTHDLR.*/DLL =
	      UART_LOAD_DLL(diviserOptimal);
	    /* Then reset DLAB bit */
	    ((LPC_UART1_TypeDef *)UARTx)->LCR &=
	      (~UART_LCR_DLAB_EN) & UART_LCR_BITMASK;
	    ((LPC_UART1_TypeDef *)UARTx)->FDR = 
	      (UART_FDR_MULVAL(mulFracDivOptimal) |
 ...
 *			the specified  parameters in the UART_ConfigStruct.
 ...
 *			Pointer to a UART_CFG_Type structure
  ...
  UART_CFG_Type *UART_ConfigStruct)
{
    ...
    if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1) {
	/* FIFOs are empty */
	((LPC_UART1_TypeDef *)UARTx)->/*IIFCR.*/FCR =
    ...
    if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1) {
	tmp = (((LPC_UART1_TypeDef *)UARTx)->LCR &
	  (UART_LCR_DLAB_EN | UART_LCR_BREAK_EN)) & UART_LCR_BITMASK;
    } else {
	tmp = (UARTx->LCR & (UART_LCR_DLAB_EN | UART_LCR_BREAK_EN)) &
	  UART_LCR_BITMASK;
    }
...
/*********************************************************************
 * @brief		De-initializes the UARTx peripheral registers
...
/**************************************************************************
* @brief		Fills each UART_InitStruct member with
*			its default value:
* 			- 9600 bps
* 			- 8-bit data
* 			- 1 Stopbit
* 			- None Parity
* @param[in]	UART_InitStruct
*			Pointer to a UART_CFG_Type structure
  ...
  uint8_t Data)
{
    ...
    if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1) {
    	((LPC_UART1_TypeDef *)UARTx)->/*RBTHDLR.*/THR =
	  Data & UART_THR_MASKBIT;
    } else {
  ...
  LPC_UART_TypeDef* UARTx)
{
    ...
    if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1) {
	return (((LPC_UART1_TypeDef *)UARTx)->/*RBTHDLR.*/RBR &
	  UART_RBR_MASKBIT);
    } else {
...
/*********************************************************************
 * @brief		Force BREAK character on UART line, output
 *			pin UARTx TXD is forced to logic 0.
 * @param[in]	UARTx	UART peripheral selected, should be:
 *  			- LPC_UART0: UART0 peripheral
 * 			- LPC_UART1: UART1 peripheral
 * 			- LPC_UART2: UART2 peripheral
 ...
 * 			- LPC_UART3: UART3 peripheral
 * @param[in]	UARTIntCfg
 *			Specifies the interrupt flag, should be one of
 *			the following:
 ...
 *			- UART1_INTCFG_CTS:
 *			  CTS1 signal transition interrupt en. (UART1 only)
 *			- UART_INTCFG_ABEO:
 *			  Enables the end of auto-baud interrupt
 *			- UART_INTCFG_ABTO:
 *			  Enables the auto-baud time-out interrupt
 * @param[in]	NewState
 *			New state of specified UART interrupt type,
 *			should be:
  ...
  FunctionalState NewState)
{
	...
	if ((LPC_UART1_TypeDef *) UARTx == LPC_UART1) {
	    ((LPC_UART1_TypeDef *)UARTx)->/*DLIER.*/IER &=
...
/********************************************************************
 * @brief 		Get current value of Line Status register in
 *			UART peripheral.
 * @param[in]	UARTx	UART peripheral selected, should be:
 *  			- LPC_UART0: UART0 peripheral
 * 			- LPC_UART1: UART1 peripheral
 * 			- LPC_UART2: UART2 peripheral
 * 			- LPC_UART3: UART3 peripheral
 * @return		Current value of Line Status register in UART
 *			peripheral.
 * Note: The return value of this function must be ANDed with each member
 * in UART_LS_Type enumeration to determine current flag status
 * corresponding to each Line status type. Because some flags in
 * Line Status register will be cleared after reading, the next
 * reading Line Status register could not be correct. So this
 * function used to read Line status register in one time only,
 ...
 * 			- LPC_UART2: UART2 peripheral
 * 			- LPC_UART3: UART3 peripheral
 * @return		Current value of UART UIIR register in UART
 *			peripheral.
 *********************************************************************/
 ...
 * 			contains specified information about FIFO
 *			configuration
 * @return 		none
 **********************************************************************/
void UART_FIFOConfig(
  LPC_UART_TypeDef *UARTx,
...
/**************************************************************************
 * @brief		Fills each UART_FIFOInitStruct member with its
 *			default value:
 ...
 *			Pointer to a UART_FIFO_CFG_Type
 *			structure which will be initialized.
 * @return		None
 **************************************************************************/
void UART_FIFOConfigStructInit(
  UART_FIFO_CFG_Type *UART_FIFOInitStruct)
{
    ...
    /* UART_FIFOInitStruct->FIFO_State = ENABLE; */
}
 ...
 * 			- LPC_UART2: UART2 peripheral
 * 			- LPC_UART3: UART3 peripheral
 * @param[in]	ABConfigStruct
 *			A pointer to UART_AB_CFG_Type structure that
 * 			contains specified information about UART
 * 			auto baudrate configuration
 * @param[in]	NewState
			New State of Auto baudrate activity, should be:
 ...
 * 			- LPC_UART3: UART3 peripheral
 * @param[in]	ABIntType
 *			Type of auto-baud interrupt, should be:
 ...
 * 			- LPC_UART3: UART3 peripheral
 * @param[in]	NewState
 *			New State of Tx transmission function, should be:
...
void UART_TxCmd(LPC_UART_TypeDef *UARTx, FunctionalState NewState)
{
	...
	if (((LPC_UART1_TypeDef *)UARTx) == LPC_UART1) {
	    ((LPC_UART1_TypeDef *)UARTx)->TER &=
	      (~UART_TER_TXEN) & UART_TER_BITMASK;
	} else {
...
/*********************************************************************
 * @brief		Enable or disable inverting serial input
 *			function of IrDA on UART peripheral.
 * @param[in]	UARTx	UART peripheral selected, should be LPC_UART3 (only)
 * @param[in]	NewState
 *			New state of inverting serial input, should be:
 * 			- ENABLE: Enable this function.
 * 			- DISABLE: Disable this function.
 * @return		none
 **********************************************************************/
void UART_IrDAInvtInputCmd(
  LPC_UART_TypeDef* UARTx,
  FunctionalState NewState)
{
 ...
 * @param[in]	UARTx	UART peripheral selected, should be LPC_UART3 (only)
 * @param[in]	NewState
 *			New state of IrDA function, should be:
 * 			- ENABLE: Enable this function.
 * 			- DISABLE: Disable this function.
 * @return		none
 **********************************************************************/
...
/*********************************************************************
 * @brief		Configure Pulse divider for IrDA function on
 *			UART peripheral.
 * @param[in]	UARTx	UART peripheral selected, should be LPC_UART3 (only)
 * @param[in]	PulseDiv
 *			Pulse Divider value from Peripheral clock,
...
/*********************************************************************
 * @brief		Force pin DTR/RTS corresponding to given state
 *			(Full modem mode)
 ...
 * @param[in]	NewState
 *			New State of DTR/RTS pin, should be:
 * 			- INACTIVE: Force the pin to inactive signal.
 *			- ACTIVE: Force the pin to active signal.
 * @return		none
 **********************************************************************/
void UART_FullModemForcePinState(
 ...
 * @param[in]	NewState
 *			New State of this mode, should be:
 * 			- ENABLE: Enable this mode.
 *			- DISABLE: Disable this mode.
 * @return		none
 **********************************************************************/
void UART_FullModemConfigMode(
  LPC_UART1_TypeDef *UARTx,
 ...
 **********************************************************************/
uint8_t UART_FullModemGetStatus(
  LPC_UART1_TypeDef *UARTx)
{
    CHECK_PARAM(PARAM_UART1_MODEM(UARTx));
...
/*********************************************************************
 * @brief		Configure UART peripheral in RS485 mode according
 *			to the specified parameters in RS485ConfigStruct.
 * @param[in]	UARTx	LPC_UART1 (only)
 * @param[in]	RS485ConfigStruct
 *			Pointer to a UART1_RS485_CTRLCFG_Type structure
 *                    	that contains the configuration information for
 *			specified UART in RS485 mode.
 * @return		None
 **********************************************************************/
void UART_RS485Config(
  LPC_UART1_TypeDef *UARTx,
  UART1_RS485_CTRLCFG_Type *RS485ConfigStruct)
{
    ...
    CHECK_PARAM(PARAM_UART1_MODEM(UARTx));
    CHECK_PARAM(PARAM_FUNCTIONALSTATE(
      RS485ConfigStruct->AutoAddrDetect_State));
    CHECK_PARAM(PARAM_FUNCTIONALSTATE(
      RS485ConfigStruct->AutoDirCtrl_State));
    CHECK_PARAM(PARAM_UART1_RS485_CFG_DELAYVALUE(
      RS485ConfigStruct->DelayValue));
    CHECK_PARAM(PARAM_SETSTATE(RS485ConfigStruct->DirCtrlPol_Level));
    CHECK_PARAM(PARAM_UART_RS485_DIRCTRL_PIN(
      RS485ConfigStruct->DirCtrlPin));
    CHECK_PARAM(PARAM_UART1_RS485_CFG_MATCHADDRVALUE(
      RS485ConfigStruct->MatchAddrValue));
    CHECK_PARAM(PARAM_FUNCTIONALSTATE(
      RS485ConfigStruct->NormalMultiDropMode_State));
    CHECK_PARAM(PARAM_FUNCTIONALSTATE(RS485ConfigStruct->Rx_State));

    tmp = 0;
    /* If Auto Direction Control is enabled:
     *  This function is used in Master mode: */
    ...
    /* Fill delay time: */
    UARTx->RS485DLY =
       RS485ConfigStruct->DelayValue & UART1_RS485DLY_BITMASK;
	...
	/* Fill Match Address: */
	UARTx->ADRMATCH =
	  RS485ConfigStruct->MatchAddrValue & UART1_RS485ADRMATCH_BITMASK;
    }
    ...
    /* Enable Parity function and leave parity in stick '0' parity
     * as default: */
 ...
 * @param[in]	UARTx	LPC_UART1 (only)
 * @param[in]	NewState
 *			New State of command, should be:
 * 			- ENABLE: Enable this function.
 * 			- DISABLE: Disable this function.
 * @return		None
 **********************************************************************/
void UART_RS485ReceiverCmd(
  LPC_UART1_TypeDef *UARTx,
  FunctionalState NewState)
{
    if (NewState == ENABLE) {
...
/*********************************************************************
 * @brief		Send data on RS485 bus with specified parity
 *			stick value (9-bit mode).
 * @param[in]	UARTx	LPC_UART1 (only)
 * @param[in]	pDatFrm Pointer to data frame.
 * @param[in]	size	Size of data.
 * @param[in]	ParityStick
 *			Parity Stick value, should be 0 or 1.
 * @return		None
 **********************************************************************/
